...
def send_move(txt):
    ...
    users = itchat.search_friends(name='allinblockchain')   # 使用备注名来查找实际用户名
    #获取好友全部信息,返回一个列表,列表内是一个字典
    print(users)
    #获取`UserName`,用于发送消息
    userName = users[0]['UserName']
    itchat.send(txt,toUserName = userName)
    print('succeed')

if __name__=='__main__':
    itchat.auto_login(hotReload=True)  # 首次扫描登录后后续自动登录
    # sched_time = datetime.datetime(2019,11,6,18,50,1)   #设定初次触发事件的事件点
