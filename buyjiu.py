...
def buyjiu(data):
    ...
    function encryptByDES(key,message) {//传入加密的内容
       //把私钥转换成16进制的字符串
    ...
    s = requests.Session()
    headers = {'User-Agent': 'Mozilla/5.0 (iPhone; CPU iPhone OS 12_1_4 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/16D57',
               'Referer': 'http://www.gzairports.com:11111/order/index.html?from=groupmessage&isappinstalled=0'
               }

    res = s.post('http://www.gzairports.com:11111/searchOrderAppointmentSettings.action', headers=headers)

    sj = res.json()['result']['extend']['appointmentSettings'][0]
    key = str(sj['deliverTimeApart']) + str(sj['remainApoointmentPersonCountApart'])
    message = str(sj['deliverTimeDepart']) + str(sj['remainApoointmentPersonCountDepart'])
